...
- Step1: UI will take the input:
  - Template Name
  - Device family
  - Template Content
  - Template-type (SET/XML)
...
- Step3: Store the template content and variables associated.
...
- Step3: UI should call `push_config_template` job with stored config_template_uuid, global variable values, list of device_uuid and device level variable values.
- Step4: Job will invoke the an ansible playbook to push the config to device(s) by
  ...
  - get the CLI content and push the configuration to the selected device(s).
...
- Workflow for creation of template by template-name, device-family, template-content, template-type and variables.
...
- New changes in fabric.xsd for ConfigTemplate object creation with template-name, template-content, device-family, template-type (SET/XML), device-vars and global-vars.
- New `push_config_template` job which calls ansible role `push_config_template` to render the template and push configuration to managed devices according to provided global/device variables.
...
## 15.1 Sanity tests
- Call `/validate-template` api by providing template-name, template-content, device-family, template-type (SET/XML) and get associated variables.
- Create ConfigTemplate object by providing template-name, template-content, device-family, template-type (SET/XML) and variables values.
- Call `push_config_template` job with global variables values for both devices.
- Job should be succeeded and config correct config should be pushed.
